<x-app-layout>
                    ...
                    <div class="bg-white rounded-xl shadow-lg p-6 mb-6">
                        <div class="flex items-center justify-between mb-6">
                                ...
                                <div>
                                    ...
                                    <div class="flex items-center space-x-3 mt-1">
                                        <span class="inline-flex items-center px-4 py-2 rounded-full text-sm font-bold bg-blue-100 text-blue-800">
                                            ...
                                            Soal <span id="question-number">{{ $currentIndex + 1 }}</span> dari {{ $allSoals->count() }}
                                        </span>
                                        <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-gray-100 text-gray-700">
                                            <i class="fas fa-tag mr-1"></i>
                                            {{ $soal->category->name ?? 'General' }}
                                        </span>
                                    </div>
                                </div>
                            </div>
                            <div class="flex items-center space-x-4">
                                <!-- Timer (Countdown) -->
                                @php
                                    $displayRemainingSeconds = $remainingSeconds ?? ($tryout->duration_minutes * 60);
                                    $displayMinutes = floor($displayRemainingSeconds / 60);
                                    $displaySeconds = $displayRemainingSeconds % 60;
                                    $displayTime = sprintf('%02d:%02d', $displayMinutes, $displaySeconds);
                                @endphp
                                <div id="countdown-timer" class="bg-gradient-to-r from-orange-500 to-red-500 text-white px-6 py-3 rounded-xl font-bold flex items-center shadow-lg">
                                    <i class="fas fa-clock mr-2"></i>
                                    <span id="countdown-display">{{ $displayTime }}</span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="p-8">
                            ...
                            <form action="{{ route('tryouts.answer', $tryout) }}" method="POST" id="answer-form">
                                ...
                                <div class="space-y-4 mb-8" id="answer-options">
                                    @foreach($soal->getAvailableOptions() as $option)
                                        @php
                                            $optionKey = $option->option_key;
                                            $isSelected = $existingAnswer && $existingAnswer->jawaban === $optionKey;
                                        @endphp
                                        <label class="group flex items-start p-6 border-2 border-gray-200 rounded-2xl hover:border-blue-400 hover:shadow-xl cursor-pointer transition-all duration-300 {{ $isSelected ? 'bg-blue-50 border-blue-500 shadow-xl' : '' }}">
                                            <input type="radio" name="jawaban" value="{{ $optionKey }}" class="mt-1 mr-5 text-blue-600 focus:ring-blue-500 w-5 h-5" {{ $isSelected ? 'checked' : '' }}>
                                            <div class="flex-1">
                                                <div class="flex items-start">
                                                    ...
                                                    <div class="flex-1">
                                                        <p class="text-gray-800 leading-relaxed whitespace-pre-line text-base">{{ $option->option_text }}</p>
                                                    </div>
                                                </div>
                                            </div>
                                        </label>
    ...
    <script>
        // Timer functionality (countdown in main content)
        // Use remaining seconds from server (persists across page refreshes)
        let timeLeft = {{ $remainingSeconds ?? ($tryout->duration_minutes * 60) }};
        ...
        function updateTimer() {
            ...
            if (timeLeft <= 300) { // 5 minutes left
                $countdownTimer.removeClass().addClass('bg-gradient-to-r from-red-600 to-red-700 text-white px-6 py-3 rounded-xl font-bold flex items-center shadow-lg animate-pulse');
            } else if (timeLeft <= 600) { // 10 minutes left
                $countdownTimer.removeClass().addClass('bg-gradient-to-r from-yellow-500 to-orange-500 text-white px-6 py-3 rounded-xl font-bold flex items-center shadow-lg');
            }
            ...
            $.ajax({
                ...
                success: function(data) {
                    if (data.success) {
                        ...
                        const currentQuestionNumber = parseInt($('#question-number').text());
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    if (data.success) {
                        ...
                        console.log('Remaining seconds from server:', data.remaining_seconds);

                        // Update timer with remaining seconds from server
                        if (data.remaining_seconds !== undefined) {
                            timeLeft = data.remaining_seconds;
                            console.log('⏱️ Timer updated from server:', timeLeft, 'seconds');
                        }
        ...
        function updateQuestionDisplay(question, index, answeredQuestions = []) {
            // Update header with jQuery (using sticky header IDs)
            ...
            updateNavigationColors(index + 1, question.id, answeredQuestions);
        }
        ...
        function updateNavigationColors(currentQuestionNumber, currentSoalId, answeredQuestions = []) {
            ...
            $('.nav-question').each(function(index) {
                ...
                console.log('🔍 Processing Question ' + questionNum + ' (ID: ' + soalId + ')');

                // Check if answered - use multiple methods to check
                ...
                if (soalId) {
                    isAnswered = answeredQuestions.includes(soalId.toString()) ||
                               ...
                               globalAnsweredQuestions.some(gaq => gaq.questionNum === questionNum);
                }

                // Check if this question is in global answered questions
                const isGlobalAnswered = globalAnsweredQuestions.some(gaq =>
                    gaq.questionNum === questionNum || gaq.soalId === soalId?.toString()
                );

                // Use OR to combine both answered status checks
                const finalIsAnswered = isAnswered || isGlobalAnswered;

                console.log(`📊 Question ${questionNum}: isAnswered=${isAnswered}, isGlobalAnswered=${isGlobalAnswered}, final=${finalIsAnswered}`);
            ...
            console.log('✅ Navigation colors update complete');
        }
