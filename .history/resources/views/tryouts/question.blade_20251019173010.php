<x-app-layout>
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
            ...
            <div class="relative">
                <div class="w-full bg-white/10 rounded-full h-3">
                    <div class="bg-gradient-to-r from-green-400 to-emerald-500 h-3 rounded-full transition-all duration-500 shadow-lg" style="width: {{ (($currentIndex + 1) / $allSoals->count()) * 100 }}%"></div>
                </div>
                <div class="flex justify-between mt-2">
                    <span class="text-white/80 text-sm">{{ number_format((($currentIndex + 1) / $allSoals->count()) * 100, 1) }}% Complete</span>
                    <span class="text-white/80 text-sm">{{ $currentIndex + 1 }} / {{ $allSoals->count() }} soal</span>
                </div>
            </div>
        </div>
    </div>
            ...
            <div class="grid grid-cols-12 gap-6">
                        ...
                        <div class="grid grid-cols-10 gap-1 mb-6" id="navigation-grid">
                            @foreach($allSoals as $index => $item)
                                ...
                                <button type="button" data-question="{{ $item->nomor_soal }}" data-soal-id="{{ $item->id }}"
                                   class="nav-question w-full aspect-square rounded-md flex items-center justify-center text-xs font-black transition-all duration-300 hover:scale-105 shadow-sm
                                   {{ $index === $currentIndex ? 'bg-gradient-to-r from-blue-600 to-blue-700 text-white shadow-md ring-2 ring-blue-300 transform scale-105' :
                                      ($answered ? 'bg-gradient-to-r from-green-500 to-green-600 text-white shadow-sm hover:shadow-md' : 'bg-gray-400 text-white hover:bg-gray-500 shadow-sm hover:shadow-sm') }}">
                                    {{ $item->nomor_soal }}
                                </button>
                            @endforeach
                        </div>
                        ...
                        <div class="bg-gradient-to-br from-gray-50 to-gray-100 rounded-xl p-6 border border-gray-200">
                            <div class="text-center">
                                <div class="text-3xl font-black text-gray-900 mb-2">
                                    {{ \App\Models\JawabanUser::where(['user_id' => auth()->id(), 'tryout_id' => $tryout->id])->count() }}/{{ $allSoals->count() }}
                                </div>
                                <div class="text-sm font-bold text-gray-600 mb-4">Soal Dijawab</div>

                                <!-- Progress Bar -->
                                <div class="w-full bg-gray-200 rounded-full h-3">
                                    <div class="bg-gradient-to-r from-blue-500 to-blue-600 h-3 rounded-full transition-all duration-500 shadow-inner" style="width: {{ ((\App\Models\JawabanUser::where(['user_id' => auth()->id(), 'tryout_id' => $tryout->id])->count() / $allSoals->count()) * 100) }}%"></div>
                                </div>
                                <div class="mt-2">
                                    @php
                                    $answeredCount = \App\Models\JawabanUser::where(['user_id' => auth()->id(), 'tryout_id' => $tryout->id])->count();
                                    $percentage = ($answeredCount / $allSoals->count()) * 100;
                                @endphp
                                <span class="text-xs font-bold text-gray-700">{{ number_format($percentage, 1) }}% Selesai</span>
                                </div>
                            </div>
                        </div>
                ...
                <div class="col-span-8">
                    ...
                    <div class="bg-white rounded-xl shadow-lg p-6 mb-6">
                        ...
                        <div>
                            <div class="w-full bg-gray-200 rounded-full h-3">
                                <div class="bg-gradient-to-r from-blue-500 to-blue-600 h-3 rounded-full transition-all duration-500 shadow-inner" style="width: {{ (($currentIndex + 1) / $allSoals->count()) * 100 }}%"></div>
                            </div>
                            <div class="flex justify-between mt-2">
                                <span class="text-sm font-bold text-gray-700">Progress: {{ number_format((($currentIndex + 1) / $allSoals->count()) * 100, 1) }}%</span>
                                <span class="text-sm text-gray-500">{{ $currentIndex + 1 }} / {{ $allSoals->count() }} soal</span>
                            </div>
                        </div>
                    </div>
    ...
    <script>
                ...
                success: function(data) {
                    if (data.success) {
                        ...
                        if (data.total_answered !== undefined) {
                            ...
                            $('.bg-gradient-to-r.from-green-400.to-emerald-500').css('width', progressPercentage + '%');
                            $('.text-white\\/80.text-sm').first().text(progressPercentage + '% Complete');
                            $('.text-white\\/80.text-sm').last().text(`${answeredCount} / ${totalQuestions} soal`);

                            // Update sidebar stats
                            $('.text-3xl.font-black.text-gray-900.mb-2').first().text(`${answeredCount}/${totalQuestions}`);
                            $('.bg-gradient-to-r.from-blue-500.to-blue-600').css('width', progressPercentage + '%');
                            $('.text-xs.font-bold.text-gray-700').first().text(progressPercentage + '% Selesai');
                        }
        ...
        function updateQuestionDisplay(question, index, answeredQuestions = []) {
            ...
            // Update header progress bar (answered questions based) - use more specific selectors
            const $headerProgressBar = $('.sticky.top-0 .bg-gradient-to-r.from-green-400.to-emerald-500');
            const $headerProgressText = $('.sticky.top-0 .text-white\\/80.text-sm').first();
            const $headerProgressCount = $('.sticky.top-0 .text-white\\/80.text-sm').last();

            console.log('🔍 Looking for header progress elements...');
            console.log('Header progress bar found:', $headerProgressBar.length > 0);
            console.log('Header progress text found:', $headerProgressText.length > 0);
            console.log('Header progress count found:', $headerProgressCount.length > 0);

            if ($headerProgressBar.length) {
                $headerProgressBar.css('width', progressPercentage + '%');
                console.log('✅ Header progress bar updated to:', progressPercentage + '%');
            } else {
                console.log('❌ Header progress bar not found - trying alternative selector');
                // Try alternative selector
                const $altHeaderBar = $('.bg-gradient-to-br.from-blue-900 .bg-gradient-to-r.from-green-400.to-emerald-500');
                if ($altHeaderBar.length) {
                    $altHeaderBar.css('width', progressPercentage + '%');
                    console.log('✅ Alternative header progress bar updated to:', progressPercentage + '%');
                }
            }

            if ($headerProgressText.length) {
                $headerProgressText.text(progressPercentage + '% Complete');
                console.log('✅ Header progress text updated to:', progressPercentage + '%');
            }

            if ($headerProgressCount.length) {
                $headerProgressCount.text(`${answeredCount} / ${totalQuestions} soal`);
                console.log('✅ Header progress count updated to:', `${answeredCount} / ${totalQuestions} soal`);
            }

            // Update main content progress bar (current position based)
            const $mainProgressBar = $('.bg-gradient-to-r.from-blue-500.to-blue-600').first();
            const $mainProgressText = $('.text-sm.font-bold.text-gray-700').first();
            const $mainProgressCount = $('.text-sm.text-gray-500').first();

            if ($mainProgressBar.length) {
                $mainProgressBar.css('width', currentPositionPercentage + '%');
                console.log('✅ Main progress bar updated to:', currentPositionPercentage + '%');
            }

            if ($mainProgressText.length) {
                $mainProgressText.text('Progress: ' + currentPositionPercentage + '%');
            }

            if ($mainProgressCount.length) {
                $mainProgressCount.text(`${index + 1} / ${totalQuestions} soal`);
            }
            ...
            $('.nav-question').each(function(index) {
                ...
                if (questionNum === currentQuestionNumber) {
                    ...
                    if (isAnswered) {
                        ...
                        // Not answered - red
                        $btn.addClass('bg-gradient-to-r from-red-500 to-red-600 text-white hover:from-red-600 hover:to-red-700 shadow-sm hover:shadow-sm');
                        console.log('Question ' + questionNum + ' → RED (unanswered)');
                    }
                }
            });
        }
        ...
        $(document).ready(function() {
            ...
            $('.nav-question').each(function() {
                ...
                // Check if button has green classes from Laravel backend
                // FIXED: Check for both gradient classes to ensure it's truly answered
                if (classes.includes('from-green-500') && classes.includes('to-green-600')) {
                    globalAnsweredQuestions.push({
                        questionNum: questionNum,
                        soalId: soalId.toString()
                    });
                    console.log('✅ Found answered question from backend:', questionNum);
                } else {
                    console.log(`❌ Question ${questionNum} not answered on backend`);
                }
            });
        ...
        function updateNavigationColorsImmediate(currentQuestionNumber) {
            ...
            $('.nav-question').each(function() {
                ...
                if (questionNum === currentQuestionNumber) {
                    if (isAnswered) {
                        // Current + Answered = GREEN
                        console.log(`🟢 Question ${questionNum}: Current + Answered = GREEN`);
                        ...
                        // Current + Not Answered = BLUE
                        console.log(`🔵 Question ${questionNum}: Current + Not Answered = BLUE`);
