<x-app-layout>
    ...
    <div class="py-6">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            {{-- FORCE SHOW CONTENT --}}
            <div class="bg-red-500 text-white p-8 mb-4 text-center text-2xl font-bold">
                🚨 TEST: JIKA INI MUNCUL BERARTI VIEW BERFUNGSI! 🚨
            </div>

            {{-- DEBUG INFO --}}
            <div class="bg-yellow-100 border-l-4 border-yellow-500 p-4 mb-4">
                <p><strong>DEBUG:</strong> total_tryouts_completed = {{ $stats['total_tryouts_completed'] ?? 'NULL' }}</p>
                <p><strong>DEBUG:</strong> recentTryouts->count() = {{ $recentTryouts->count() }}</p>
                <p><strong>DEBUG:</strong> Kondisi empty state = {{ ($stats['total_tryouts_completed'] == 0 && $recentTryouts->count() == 0) ? 'TRUE' : 'FALSE' }}</p>
            </div>
            ...
            @if($stats['total_tryouts_completed'] == 0 && $recentTryouts->count() == 0)
                <div class="text-center py-16">
                    ...
                    <div class="bg-gradient-to-r from-blue-600 to-orange-600 rounded-2xl p-8 text-white max-w-2xl mx-auto mb-8">
                        <h3 class="text-2xl font-bold mb-4">🚀 Mulai Sekarang!</h3>
                    ...
                    <div class="text-gray-500 text-sm">
                        💡 <strong>Tips:</strong> Mulai dengan tryout kategori TWK untuk mengukur kemampuan dasarmu!
                    </div>
                </div>
            @else
