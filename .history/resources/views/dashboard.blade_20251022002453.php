...
@php
    // Define helper variables for backward compatibility - data from controller
    $totalAvg = isset($performanceByCategory) ? $performanceByCategory->avg('avg_score') : 0;

    // Count categories based on controller data
    $excellentCount = isset($strongCategories) ? $strongCategories->count() : 0;
    $needsImprovementCount = isset($weakCategories) ? $weakCategories->count() : 0;

    // Prepare critical categories for recommendations - using controller data
    ...
    if(isset($performanceByCategory) && $performanceByCategory->count() > 0) {
        foreach($performanceByCategory as $category) {
            ...
            $passingScore = $passingGrades[$categoryName] ?? ($passingGrades['DEFAULT'] ?? 85);
                                                    ...
                                                    <div class="flex justify-between items-center p-2 bg-white rounded border">
                                                        <div>
                                                            <div class="flex items-center gap-1">
                                                                <span class="text-sm font-medium text-orange-700">📋 {{ $category['kategori'] }}</span>
                                                            </div>
                                                            <div class="text-xs text-gray-500">Score: {{ $category['avg_score'] }}</div>
                                                        </div>
                                                        <div class="text-right">
                                ...
                                @if($totalAvg >= 80)
                                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        <div class="bg-white/80 rounded-lg p-4">
                                            ...
                                            <p class="text-sm text-gray-700 mb-4">Performa Anda luar biasa! Fokus pada pemeliharaan dan tingkatkan kecepatan pengerjaan.</p>
                                        </div>
                                    </div>
                                    ...
                                    <div class="text-center mt-6">
                                        <a href="{{ route('performance.analysis') }}"
                                           class="inline-flex items-center px-6 py-3 bg-gradient-to-r from-indigo-600 to-purple-600 text-white font-semibold rounded-lg hover:from-indigo-700 hover:to-purple-700 transition-all duration-300 transform hover:scale-105 shadow-lg">
                                            <i class="fas fa-chart-line mr-2"></i>
                                            <span>Lihat Analisis Performa Detail</span>
                                        </a>
                                    </div>
                                @else
                                        <div class="bg-white/80 rounded-lg p-4">
