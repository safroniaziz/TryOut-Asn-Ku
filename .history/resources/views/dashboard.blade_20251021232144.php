...
@php
    ...
    if(isset($performanceByCategory) && $performanceByCategory->count() > 0) {
        ...
        $passedCategories = collect($performanceByCategory)->filter(function($item) use ($passingGrades) {
            $categoryName = strtoupper($item['kategori']);
            $passingScore = $passingGrades[$categoryName] ?? $passingGrades['DEFAULT'];
            return $item['avg_score'] >= $passingScore;
        });

        $failedCategories = collect($performanceByCategory)->filter(function($item) use ($passingGrades) {
            $categoryName = strtoupper($item['kategori']);
            $passingScore = $passingGrades[$categoryName] ?? $passingGrades['DEFAULT'];
            return $item['avg_score'] < $passingScore;
        });

        $totalAvg = collect($performanceByCategory)->avg('avg_score');
            ...
            foreach($performanceByCategory as $category) {
                ...
                if($category['avg_score'] < $passingScore) {
                    ...
                    $criticalCategories[] = [
                        ...
                        'gap' => round($selisih, 1),
                        'type' => isset($passingGrades[$categoryName]) ? ($categoryName == 'TWK' || $categoryName == 'TIU' || $categoryName == 'TKP' ? 'CPNS' : 'PPPK') : 'PPPK'
                    ];
                }
            }
        }
                                ...
                                <div class="flex items-center space-x-6">
                                    <div class="text-center">
                                        <div class="text-3xl font-bold text-transparent bg-gradient-to-r from-orange-600 to-red-600 bg-clip-text">
                                            {{ number_format(collect($performanceByCategory)->avg('avg_score'), 1) }}
                                        </div>
                                    ...
                                    <div class="text-center">
                                        <div class="text-3xl font-bold text-transparent bg-gradient-to-r from-blue-600 to-purple-600 bg-clip-text">
                                            {{ collect($performanceByCategory)->filter(fn($item) => $item['avg_score'] >= 65)->count() }}
                                        </div>
                            ...
                            <div class="bg-white rounded-xl p-6 border border-gray-200 shadow-sm">
                                <div class="mb-6">
                                    ...
                                    <div class="text-center mt-3">
                                        <div class="text-sm text-gray-600">Skor Rata-rata</div>
                                        <div class="text-2xl font-bold text-purple-600">{{ number_format(collect($performanceByCategory)->avg('avg_score'), 1) }}/100</div>
                                    </div>
                                </div>
                                ...
                                <div class="grid grid-cols-3 gap-3 mt-6">
                                    ...
                                    <div class="text-center p-3 bg-green-50 rounded-lg border border-green-200">
                                        <div class="text-lg font-bold text-green-600">{{ round((collect($performanceByCategory)->filter(fn($item) => $item['avg_score'] >= 65)->count() / max($performanceByCategory->count(), 1)) * 100) }}%</div>
                            ...
                            <div class="bg-white rounded-xl p-6 border border-gray-200 shadow-sm">
                                <div class="mb-6">
                                    ...
                                    <div class="text-right">
                                        ...
                                        <div class="text-xl font-bold text-green-600">{{ collect($performanceByCategory)->sortByDesc('avg_score')->first()['kategori'] ?? '-' }}</div>
                                    </div>
                                </div>
                                ...
                                <div class="space-y-4">
                                    <div class="bg-gray-50 rounded-lg p-4">
                                        ...
                                        <div class="grid grid-cols-3 gap-3">
                                            <div class="text-center">
                                                <div class="text-lg font-bold text-blue-600">{{ collect($performanceByCategory)->pluck('avg_score')->max() }}/100</div>
                                                <div class="text-xs text-gray-600">Tertinggi</div>
                                            </div>
                                            <div class="text-center">
                                                <div class="text-lg font-bold text-orange-600">{{ collect($performanceByCategory)->pluck('avg_score')->min() }}/100</div>
                                                <div class="text-xs text-gray-600">Terendah</div>
                                            </div>
                                            <div class="text-center">
                                                <div class="text-lg font-bold text-purple-600">{{ collect($performanceByCategory)->pluck('avg_score')->max() - collect($performanceByCategory)->pluck('avg_score')->min() }}</div>
                                    ...
                                    <div class="bg-gray-50 rounded-lg p-4">
                                        ...
                                        <div class="grid grid-cols-2 gap-3">
                                            <div class="text-center">
                                                <div class="text-lg font-bold text-green-600">{{ collect($performanceByCategory)->filter(fn($item) => $item['avg_score'] >= 60)->count() }}</div>
                                ...
                                <div class="space-y-6">
                                    ...
                                    <div class="bg-gradient-to-r from-purple-50 to-indigo-50 rounded-lg p-4 border border-purple-200">
                                        ...
                                        <div class="grid grid-cols-2 gap-4">
                                            <div class="text-center">
                                                <div class="text-2xl font-bold text-purple-600">{{ number_format(collect($performanceByCategory)->avg('avg_score'), 1) }}</div>
                                                <div class="text-sm text-gray-600">Skor Rata-rata</div>
                                            </div>
                                            <div class="text-center">
                                                <div class="text-2xl font-bold text-green-600">{{ round((collect($performanceByCategory)->filter(fn($item) => $item['avg_score'] >= 60)->count() / max($performanceByCategory->count(), 1)) * 100) }}%</div>
                                    ...
                                    <div class="bg-gradient-to-r from-blue-50 to-cyan-50 rounded-lg p-4 border border-blue-200">
                                        ...
                                        <div class="space-y-2">
                                            <div class="flex justify-between items-center py-2 border-b border-blue-100">
                                                ...
                                                <span class="font-bold text-green-600 text-lg">{{ collect($performanceByCategory)->filter(fn($item) => $item['avg_score'] >= 65)->count() }}</span>
                                            </div>
                                            <div class="flex justify-between items-center py-2 border-b border-blue-100">
                                                <span class="text-sm text-gray-600">Tidak Lulus (<65)</span>
                                                <span class="font-bold text-orange-600 text-lg">{{ collect($performanceByCategory)->filter(fn($item) => $item['avg_score'] < 65)->count() }}</span>
                                            </div>
                                            <div class="flex justify-between items-center py-2">
                                                <span class="text-sm text-gray-600">Range Skor</span>
                                                <span class="font-bold text-blue-600 text-lg">{{ collect($performanceByCategory)->pluck('avg_score')->min() }} - {{ collect($performanceByCategory)->pluck('avg_score')->max() }}</span>
                                            </div>
                                        </div>
                                    </div>
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                                    ...
                                    <div class="bg-gradient-to-br from-green-50 to-emerald-50 rounded-xl p-4 border border-green-200/50">
                                        ...
                                        @if(isset($strongCategories) && $strongCategories->count() > 0)
                                            <div class="space-y-2">
                                                @foreach($strongCategories->take(2) as $category => $data)
                                                    <div class="flex justify-between items-center">
                                                        <span class="text-sm font-medium text-green-700">{{ $category }}</span>
                                                        <span class="text-sm font-bold text-green-600">{{ $data['avg_score'] }}</span>
                                                    </div>
                                                @endforeach
                                            </div>
                                        @else
...
<div class="bg-gradient-to-br from-blue-50 to-indigo-50 rounded-xl p-4 border border-blue-200/50">
    ...
    @if(isset($strongCategories) && $strongCategories->count() > 0)
        <div class="space-y-2">
            <h4 class="text-xs font-semibold text-green-700 mb-2">✅ Kategori Lulus Passing Grade:</h4>
            @foreach($strongCategories->take(3) as $category)
                @php
                    $categoryName = strtoupper($category['kategori']);
                    $passingScore = $passingGrades[$categoryName] ?? $passingGrades['DEFAULT'];
                    $testType = isset($passingGrades[$categoryName]) ? ($categoryName == 'TWK' || $categoryName == 'TIU' || $categoryName == 'TKP' ? 'CPNS' : 'PPPK') : 'PPPK';
                    $testTypeIcon = ($testType == 'CPNS') ? '🏛️' : '📋';
                @endphp
                <div class="flex justify-between items-center p-2 bg-white rounded border">
                    <div>
                        <div class="flex items-center gap-1">
                            <span class="text-sm font-medium text-green-700">{{ $testTypeIcon }} {{ $category['kategori'] }}</span>
                        </div>
                        <div class="text-xs text-gray-500">Passing: {{ $passingScore }}</div>
                    </div>
                    <div class="text-right">
                        ...
                        @else
                            <div class="text-xs text-orange-600">❌ TIDAK</div>
                        @endif
                    </div>
                </div>
            @endforeach
    ...
    @else
        <p class="text-sm text-orange-600">⚠️ Belum ada kategori yang lulus passing grade</p>
    @endif
</div>
        ...
        <div class="space-y-2">
            ...
            @foreach($weakCategories->take(3) as $category)
                ...
                <div class="flex justify-between items-center p-2 bg-white rounded border">
                    <div>
                        <div class="flex items-center gap-1">
                            <span class="text-sm font-medium text-orange-700">{{ $testTypeIcon }} {{ $category['kategori'] }}</span>
                        </div>
