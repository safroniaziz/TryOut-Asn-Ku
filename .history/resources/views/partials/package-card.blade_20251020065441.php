<div class="h-full flex flex-col">
    <div class="bg-white rounded-xl shadow-md hover:shadow-lg transition-all duration-300 hover:-translate-y-1 overflow-hidden h-full flex flex-col">
        <!-- Header -->
        <div class="bg-gradient-to-r {{ $package['type'] === 'CPNS' ? 'from-blue-600 to-blue-700' : 'from-emerald-600 to-green-700' }} p-4 flex-shrink-0">
            <div class="flex items-center justify-between mb-3">
                ...
                <div class="flex items-center space-x-2">
                    ...
                    @if($package['payment_type_label'])
                        <div class="bg-gradient-to-r from-amber-500 to-orange-500 rounded-lg px-3 py-1">
                            <span class="text-white text-xs font-bold flex items-center">
                                <i class="fas fa-crown mr-1"></i>{{ $package['payment_type_label'] }}
                            </span>
                        </div>
                    @endif
                </div>
            ...
            <div class="grid grid-cols-3 gap-2 {{ $package['type'] === 'CPNS' ? 'text-blue-100' : 'text-emerald-100' }} text-sm">
                <!-- Total Questions -->
                <div class="flex items-center">
                    <i class="fas fa-file-alt mr-1.5"></i>
                    <span>{{ $package['tryouts']->sum(function($t) { return $t->soals()->count(); }) }} Soal</span>
                </div>
                <!-- Category Count -->
                <div class="flex items-center">
                        ...
                        @if(!empty($package['category_details']))
                            @foreach($package['category_details'] as $category)
                                <div class="flex items-center justify-between p-2 bg-gray-50 rounded-lg">
                                    <div class="flex items-center">
                                        <div class="w-2 h-2 {{ $package['type'] === 'CPNS' ? 'bg-blue-500' : 'bg-emerald-500' }} rounded-full mr-2"></div>
        ...
        <div class="bg-gray-50 px-4 py-3 border-t border-gray-100 flex-shrink-0">
            ...
            @if($package['is_premium_package'] && !$hasPremium)
                <a href="{{ route('subscription.premium') }}"
                   class="flex items-center justify-center w-full bg-gradient-to-r from-orange-500 to-red-500 hover:from-orange-600 hover:to-red-600 text-white font-bold py-2.5 px-6 rounded-xl transition-all duration-200 text-center shadow-md hover:shadow-lg space-x-2">
