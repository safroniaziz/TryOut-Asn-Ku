<x-app-layout>
    ...
    <div class="bg-gradient-to-br from-slate-50 to-gray-100 relative overflow-hidden">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 xl:px-12 pt-12 pb-12">
            <!-- Section Header -->
                        ...
                        <div class="relative h-full">
                            ...
                            <div class="relative bg-white rounded-2xl shadow-lg hover:shadow-xl transition-all duration-300 hover:-translate-y-1 p-6 h-full flex flex-col border border-gray-100 min-h-[280px]">
                                ...
                                <div class="flex-grow flex flex-col justify-center">
                                    ...
                                    @else
                                        <div class="text-xs text-gray-500 text-center font-medium bg-gray-50 px-3 py-1 rounded-full">Mulai!</div>
                                    @endif
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="relative h-full">
                            ...
                            <div class="relative bg-white rounded-2xl shadow-lg hover:shadow-xl transition-all duration-300 hover:-translate-y-1 p-6 h-full flex flex-col border border-gray-100 min-h-[280px]">
                                ...
                                <div class="flex-grow flex flex-col justify-center">
                                    ...
                                    @else
                                        <div class="text-xs text-gray-500 text-center font-medium bg-gray-50 px-3 py-1 rounded-full">Naik!</div>
                                    @endif
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="relative bg-white rounded-2xl shadow-lg hover:shadow-xl transition-all duration-300 hover:-translate-y-1 p-6 h-full flex flex-col border border-gray-100 min-h-[280px]">
                            ...
                            <div class="flex-grow flex flex-col justify-center">
                                @if($userTryouts->count() > 0)
                                    ...
                                    <!-- Judul Tryout -->
                                    <div class="text-sm font-bold text-gray-800 mb-1 truncate" title="{{ $tryout->title }}">
                                        {{ strlen($tryout->title) > 25 ? substr($tryout->title, 0, 25) . '...' : $tryout->title }}
                                    </div>

                                    <!-- Info Tryout -->
                                    <div class="flex items-center justify-between mb-2">
                                        <div class="flex items-center space-x-2">
                                            <span class="text-xs font-bold px-2 py-1 rounded-full {{ $tryout->type === 'CPNS' ? 'bg-blue-100 text-blue-700' : 'bg-purple-100 text-purple-700' }}">
                                                {{ $tryout->type }}
                                            </span>
                                            <span class="text-xs font-bold px-2 py-1 rounded-full {{ $tryout->isFree() ? 'bg-green-100 text-green-700' : 'bg-orange-100 text-orange-700' }}">
                                                {{ $tryout->isFree() ? 'FREE' : 'PREMIUM' }}
                                            </span>
                                        </div>
                                        <div class="text-xs text-gray-600">
                                            #{{ $latestTryout->rank ?? 'N/A' }}
                                        </div>
                                    </div>

                                    <div class="text-3xl font-black text-transparent bg-gradient-to-r from-teal-600 to-green-600 bg-clip-text mb-2 tabular-nums">{{ $latestTryout->benar }}/{{ $totalSoal }}</div>
                                    <div class="text-sm font-bold text-gray-700 uppercase tracking-wide mb-3">Jawaban Benar</div>

                                    <!-- Rincian Per Kategori -->
                                    <div class="space-y-2 mb-3">
                                        @php
                                            $jawabanByKategori = \App\Models\JawabanUser::where([
                                                'user_id' => auth()->id(),
                                                'tryout_id' => $latestTryout->tryout_id
                                            ])
                                            ->with('soal')
                                            ->get()
                                            ->groupBy('soal.kategori')
                                            ->map(function($jawabans) {
                                                $benar = $jawabans->where('is_correct', true)->count();
                                                $total = $jawabans->count();
                                                return ['benar' => $benar, 'total' => $total];
                                            });
                                        @endphp

                                        @if(isset($jawabanByKategori) && $jawabanByKategori->count() > 0)
                                            @foreach($jawabanByKategori as $kategori => $data)
                                                <div class="flex justify-between items-center text-xs">
                                                    <span class="text-gray-600 font-medium">{{ $kategori }}</span>
                                                    <span class="font-bold text-gray-800">{{ $data['benar'] }}/{{ $data['total'] }}</span>
                                                </div>
                                            @endforeach
                                        @else
                                            <div class="text-xs text-gray-500 text-center">Data kategori tidak tersedia</div>
                                        @endif
                                    </div>
                                    ...
                                    @else
                                        <div class="text-xs text-gray-500 text-center font-medium bg-gray-50 px-3 py-1 rounded-full">Lanjutkan!</div>
                                    @endif
                                @else
                                    <div class="text-3xl font-black text-transparent bg-gradient-to-r from-teal-600 to-green-600 bg-clip-text mb-2 tabular-nums">0/0</div>
                                    <div class="text-sm font-bold text-gray-700 uppercase tracking-wide mb-3">Jawaban Benar</div>
                                    <div class="text-xs text-gray-500 text-center font-medium bg-gray-50 px-3 py-1 rounded-full">Belum ada</div>
                                @endif
                            </div>
                        </div>
                    </div>
