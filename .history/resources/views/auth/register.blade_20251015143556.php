                        ...
                        <div class="mb-8">
                            <div class="flex items-center justify-between relative">
                                ...
                                <div class="absolute top-5 left-0 w-full h-0.5 bg-gray-200">
                                    <div class="h-full bg-blue-500 transition-all duration-500" id="progressLine" style="width: 25%"></div>
                                </div>

                                <!-- Step Indicators -->
                                <div class="flex items-center justify-between w-full relative z-10">
                                    <div class="step-indicator active" data-step="1">
                                        <div class="w-10 h-10 rounded-full bg-blue-500 text-white flex items-center justify-center text-sm font-semibold shadow-lg">
                                            <i class="fas fa-user"></i>
                                        </div>
                                        <p class="text-xs mt-2 font-medium text-blue-600">Personal</p>
                                    </div>
                                    <div class="step-indicator" data-step="2">
                                        <div class="w-10 h-10 rounded-full bg-gray-200 text-gray-500 flex items-center justify-center text-sm font-semibold">
                                    ...
                                    <div class="step-indicator" data-step="4">
                                        ...
                                        <p class="text-xs mt-2 font-medium text-gray-400">Selesai</p>
                                    </div>
                                </div>
                            </div>
                        </div>

                        <!-- WIZARD NAVIGATION BUTTONS (ALWAYS VISIBLE) -->
                        <div class="fixed-navigation-buttons bg-white p-4 rounded-lg border-2 border-blue-200 mb-6 shadow-md">
                            <div class="flex items-center justify-between">
                                <!-- Previous Button (Simple) -->
                                <button type="button" 
                                        id="wizardPrevBtn" 
                                        class="px-6 py-3 bg-gray-100 hover:bg-gray-200 text-gray-700 font-semibold rounded-lg transition-all duration-200 flex items-center space-x-2"
                                ...
                                <div class="text-center">
                                    <div class="text-sm font-medium text-gray-600">
                                        Step <span id="wizardCurrentStep">1</span> dari 4
                                    </div>
                                    <div class="text-xs text-blue-600 mt-1" id="wizardStepName">Data Personal</div>
                                </div>
                                
                                <!-- Next Button (Always Visible) -->
                                <button type="button" 
                                        id="wizardNextBtn" 
                                        class="px-6 py-3 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-lg transition-all duration-200 flex items-center space-x-2 shadow-lg">
                                    <span id="wizardNextText">Lanjutkan</span>
                                    <i class="fas fa-arrow-right" id="wizardNextIcon"></i>
                                </button>
                            </div>
                        </div>
...
<script>
    ...
    function initWizard() {
        console.log('🎮 Initializing Simple Wizard...');
        
        // Use the new simple buttons
        ...
        if (!prevBtn) {
            console.error('❌ Wizard Previous button not found!');
            return;
        }
        
        console.log('✅ Simple wizard buttons found successfully');
        
        nextBtn.addEventListener('click', function(e) {
            e.preventDefault();
            console.log(`🚀 Simple Next button clicked! Current step: ${currentStep}`);
        ...
        prevBtn.addEventListener('click', function(e) {
            e.preventDefault();
            console.log(`⬅️ Simple Previous button clicked! Current step: ${currentStep}`);
            prevStep();
        });

        console.log('🎯 Initial simple wizard UI update...');
        updateSimpleWizardUI();
        
        // Show success message
        alert('✅ Wizard Siap! Tombol "Lanjutkan" ada di atas form. Klik untuk melanjutkan ke step berikutnya.');
    }

    function updateSimpleWizardUI() {
        console.log(`📊 Updating simple wizard UI for step ${currentStep}`);
        
        const nextBtn = document.getElementById('wizardNextBtn');
        const prevBtn = document.getElementById('wizardPrevBtn');
        const currentStepText = document.getElementById('wizardCurrentStep');
        const stepNameText = document.getElementById('wizardStepName');
        const nextTextSpan = document.getElementById('wizardNextText');
        const nextIcon = document.getElementById('wizardNextIcon');
        
        // Update step counter
        if (currentStepText) currentStepText.textContent = currentStep;
        
        // Update step names
        const stepNames = ['Data Personal', 'Target & Aspirasi', 'Background & Analytics', 'Review & Password'];
        if (stepNameText) stepNameText.textContent = stepNames[currentStep - 1] || '';
        ...
        if (nextTextSpan && nextIcon) {
            if (currentStep === totalSteps) {
                nextTextSpan.textContent = 'Daftar Sekarang';
                nextIcon.className = 'fas fa-rocket';
                nextBtn.className = nextBtn.className.replace('bg-blue-600 hover:bg-blue-700', 'bg-green-600 hover:bg-green-700');
            } else {
                nextTextSpan.textContent = 'Lanjutkan';
                nextIcon.className = 'fas fa-arrow-right';
                nextBtn.className = nextBtn.className.replace('bg-green-600 hover:bg-green-700', 'bg-blue-600 hover:bg-blue-700');
            }
        }
        
        // Update main progress line (if exists)
        ...
        console.log(`✅ Simple wizard UI updated for step ${currentStep}`);
    }    function nextStep() {
        if (currentStep < totalSteps) {
            // Hide current step
...
<style>
    ...
    /* 🎯 Simple Navigation Buttons - Always Visible */
