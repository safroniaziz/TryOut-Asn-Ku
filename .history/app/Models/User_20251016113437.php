...
class User extends Authenticatable
{
    ...
    // Generate unique referral code dengan format ASN2024001, ASN2024002, dst
    public static function generateReferralCode($userId = null)
    {
        $year = date('Y');
        $userId = $userId ?: (static::max('id') + 1);

        // Format: ASN2024001 (min 3 digit), ASN20241000 (4 digit), dst - dynamic padding
        $minDigits = 3;
        $actualDigits = max($minDigits, strlen((string)$userId));
        return 'ASN' . $year . str_pad($userId, $actualDigits, '0', STR_PAD_LEFT);
    }
