...
class DashboardController extends Controller
{
    ...
    public function index()
    {
        ...
        // Get tryout packages for display
        try {
            $tryoutPackages = $this->getTryoutPackages($hasPremium);
        } catch (\Exception $e) {
    ...
    /**
     * Get tryout packages for dashboard display
     */
    private function getTryoutPackages(bool $hasPremium): array
    {
        // Get CPNS tryouts
        $cpnsTryouts = Tryout::active()->cpns()->orderBy('created_at', 'desc')->get();
        $cpnsPackages = $this->createSimplePackages($cpnsTryouts, 'CPNS', $hasPremium);

        // Get PPPK tryouts
        $pppkTryouts = Tryout::active()->pppk()->orderBy('created_at', 'desc')->get();
        $pppkPackages = $this->createSimplePackages($pppkTryouts, 'PPPK', $hasPremium);

        // Group by category
        $cpnsGrouped = $this->groupPackagesByCategory($cpnsPackages);
        $pppkGrouped = $this->groupPackagesByCategory($pppkPackages);
     ...
     */
    private function createSimplePackages($tryouts, string $type, bool $hasPremium): array
    {
        $packages = [];
        ...
        foreach ($tryouts as $tryout) {
            $packages[] = [
                'name' => $tryout->title,
                'type' => $type,
                'kategori' => $tryout->category,
                'tryouts' => collect([$tryout]),
                'total_questions' => $tryout->total_questions,
                'total_duration' => $tryout->duration_minutes,
                'is_free_package' => $tryout->package_type === 'free',
                'is_free' => $tryout->package_type === 'free', // Alias for compatibility
                'is_premium_package' => $tryout->package_type === 'premium',
                'is_premium' => $tryout->package_type === 'premium', // Alias for compatibility
                'is_completed_by_user' => $this->isPackageCompletedByUser(collect([$tryout]), Auth::user()),
                'user_score_in_package' => $this->getUserScoreInPackage(collect([$tryout]), Auth::user()),
                'completion_rate' => $this->getPackageCompletionRate(collect([$tryout])),
                'total_participants' => $this->getTotalParticipants(collect([$tryout])),
                'start_date' => $tryout->start_date,
                'end_date' => $tryout->end_date,
                'category_names' => [$tryout->category],
                'category_details' => [[
                    'name' => $tryout->category,
                    'question_count' => $tryout->total_questions,
                    'estimated_minutes' => $tryout->duration_minutes
                ]],
                'time_access_label' => $this->getTimeAccessLabel($tryout),
                'average_score' => $this->getAverageScore(collect([$tryout]))
            ];
        }
     ...
     */
    private function groupPackagesByCategory(array $packages): array
    {
        $grouped = [
            'all' => $packages,
            'SKD' => [],
        ...
        foreach ($packages as $package) {
            $category = $package['kategori'];

            if (in_array($category, ['TWK', 'TIU', 'TKP'])) {
                $grouped['SKD'][] = $package;
            } elseif (str_contains($category, 'SKB')) {
                $grouped['SKB'][] = $package;
            } elseif (str_contains($category, 'Teknis')) {
                $grouped['Teknis'][] = $package;
            } else {
                $grouped['Non-Teknis'][] = $package;
            }
        }
    ...
    /**
     * Check if package is completed by user
     */
    private function isPackageCompletedByUser($tryouts, $user): bool
    {
        return $user->leaderboards()
    ...
    /**
     * Get user score in package
     */
    private function getUserScoreInPackage($tryouts, $user): int
    {
        return $user->leaderboards()
            ->whereIn('tryout_id', $tryouts->pluck('id'))
            ->max('total_skor') ?? 0;
    ...
    /**
     * Get time access label
     */
    private function getTimeAccessLabel($tryout): ?string
    {
        if ($tryout->start_date && $tryout->end_date) {
            $start = \Carbon\Carbon::parse($tryout->start_date);
            $end = \Carbon\Carbon::parse($tryout->end_date);

            if ($start->isToday() && $end->isToday()) {
                return 'Hari Ini';
            } elseif ($start->isFuture() && $end->isFuture()) {
                return 'Akan Datang';
            } else {
                return 'Tersedia';
            }
    ...
    /**
     * Get average score for tryouts
     */
    private function getAverageScore($tryouts): int
    {
        $totalScore = 0;
        $count = 0;

        foreach ($tryouts as $tryout) {
            $avgScore = $tryout->leaderboards()->avg('total_skor');
            if ($avgScore) {
                $totalScore += $avgScore;
                $count++;
            }
    ...
    /**
     * Get package completion rate
     */
    private function getPackageCompletionRate($tryouts): int
    {
        $totalParticipants = 0;
        $completedParticipants = 0;

        foreach ($tryouts as $tryout) {
            $participants = $tryout->leaderboards()->count();
            $completed = $tryout->leaderboards()->whereNotNull('total_skor')->count();

            $totalParticipants += $participants;
            $completedParticipants += $completed;
        }

        return $totalParticipants > 0 ? round(($completedParticipants / $totalParticipants) * 100) : 0;
    }

    /**
     * Get total participants for tryouts
     */
    private function getTotalParticipants($tryouts): int
    {
        $totalParticipants = 0;

        foreach ($tryouts as $tryout) {
            $totalParticipants += $tryout->leaderboards()->count();
        }
