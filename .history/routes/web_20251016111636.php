...
Route::post('/api/validate-referral', function(Illuminate\Http\Request $request) {
    ...
    $user = \App\Models\User::where('my_referral_code', $referralCode)
                           ->where('id', '!=', \Illuminate\Support\Facades\Auth::id() ?? 0) // Exclude current user if logged in
...
})->name('api.validate-referral');require __DIR__.'/auth.php';
