...
class TryoutController extends Controller
{
    ...
    public function start(Request $request, Tryout $tryout)
    {
        ...
        // Get first question
        $soal = $tryout->soals()->with('category')->orderBy('nomor_soal')->first();
        ...
        if ($request->ajax()) {
            ...
            $allSoals = $tryout->soals()->with('category')->orderBy('nomor_soal')->get();
            ...
            return response()->json([
                ...
                'question' => [
                    ...
                    'kategori' => $soal->category->name ?? 'General',
    ...
    public function questionByNumber(Tryout $tryout, $questionNumber)
    {
        ...
        // Generate answer options HTML using new dynamic structure
        $answerOptionsHtml = '';
        $availableOptions = $soal->getAvailableOptions();
    ...
    {
        try {
            // Find the soal first to determine available options
            $soal = \App\Models\Soal::findOrFail($request->soal_id);

            // Get available options dynamically
            $availableOptions = $soal->getAvailableOptions()->pluck('option_key')->toArray();
            $optionsString = implode(',', $availableOptions);

            $validated = $request->validate([
                'jawaban' => "required|in:$optionsString",
        ...
        if ($request->ajax()) {
            ...
            if ($nextSoal) {
                ...
                // Last question - redirect to submit to finalize
                return response()->json([
                    'success' => true,
                    'message' => 'Selamat! TryOut telah selesai.',
                    'completed' => true,
                    'redirect' => route('tryouts.submit', $tryout)
                ]);
            }
        }
